...
from .viTAE.base_model import ViTAE_noRC_MaxPooling_basic
...
def ViTAE_noRC_MaxPooling_bias_basic_stages4_14(pretrained=True):  # adopt performer for tokens to token
    model = ViTAE_noRC_MaxPooling_basic(RC_tokens_type=['performer', 'transformer', 'transformer', 'transformer'],
                                        ...
                                        NC_depth=[2, 2, 12, 2], NC_heads=[1, 2, 4, 8], RC_heads=[1, 1, 2, 4],
                                        mlp_ratio=4., NC_group=[1, 32, 64, 128], RC_group=[1, 16, 32, 64])
...
class mergedDecoder(nn.Module):
    ...
    def __init__(self, backbone, hr_channels, enc_channels):
        ...
        """ low Resolution Branch of MODNet
            """
        enc_channels = [64, 24, 32, 96, 256]

        self.backbone = backbone

        # self.enc_channels = [64, 32, 96, 128, 256]
        self.se_block = SEBlock(enc_channels[4], enc_channels[4], reduction=4)
        self.se_block2 = SEBlock(enc_channels[3], enc_channels[3], reduction=4)
        self.conv_lr16x = Conv2dIBNormRelu(enc_channels[4], enc_channels[3], 5, stride=1, padding=2)
        self.conv_lr16x2 = Conv2dIBNormRelu(enc_channels[2], enc_channels[1], 5, stride=1, padding=2)
        self.conv_lr8x = Conv2dIBNormRelu(enc_channels[3], enc_channels[2], 5, stride=1, padding=2)
        self.conv_lr8x2 = Conv2dIBNormRelu(enc_channels[2], enc_channels[1], 5, stride=1, padding=2)
        ...
        self.tohr_enc4x = Conv2dIBNormRelu(enc_channels[0], hr_channels, 1, stride=1, padding=0)
    ...
    def forward(self, img, inference):
        # img of 512x512
        embeddings, indices, enc_features = self.backbone.forward(img)  # encoder
        enc2x, enc4x, enc8x, enc32x = enc_features[-4], enc_features[-3], enc_features[-2], enc_features[
            -1]  # 4 sets of encoder embeddings
        # print(np.shape(img))#512
        # print(np.shape(enc2x))#torch.Size([8, 64, 256, 256])
        # print(np.shape(enc4x))#torch.Size([8, 64, 128, 128])
        # print(np.shape(enc8x))#torch.Size([8, 128, 64, 64])
        # print(np.shape(enc16x))#32
        # print(np.shape(enc32x))#torch.Size([8, 256, 32, 32])

        # low-resolution branch
        # e-ASPP
        # enc32x
        enc32x = self.se_block(enc32x)  # torch.Size([8, 1280, 16, 16])
        lr16x = F.interpolate(enc32x, scale_factor=2, mode='bilinear',
                              align_corners=False)  # torch.Size([8, 1280, 32, 32])
        ...
        lr8x = F.interpolate(lr16x, scale_factor=2, mode='bilinear', align_corners=False)  # torch.Size([8, 32, 64, 64])
        lr8x = self.conv_lr8x(lr8x)  # torch.Size([8, 32, 64, 64])
        lr8x2 = F.interpolate(lr8x, scale_factor=2, mode='bilinear', align_corners=False)  # torch.Size([8, 32, 64, 64])
        lr8x2 = self.conv_lr8x2(lr8x2)  # torch.Size([8, 32, 64, 64])
        ...
        img2x = F.interpolate(img, scale_factor=1 / 2, mode='bilinear',
                              align_corners=False)  # torch.Size([8, 3, 256, 256])
        img4x = F.interpolate(img, scale_factor=1 / 4, mode='bilinear',
                              align_corners=False)  # torch.Size([8, 3, 128, 128])
        ...
        lr4x = F.interpolate(lr8x, scale_factor=2, mode='bilinear',
                             align_corners=False)  # torch.Size([8, 32, 128, 128]),lb feat

        # print(np.shape(torch.cat((hr4x, lr4x, img4x), dim=1)))#torch.Size([8, 64, 128, 128])
        hr4x = self.conv_hr4x(
            torch.cat((hr4x, lr8x, img4x), dim=1))  # torch.Size([8, 32, 128, 128]),3 enc feat+1 lb feat+ 2 img feat

        # hr4x = self.conv_hr4x(torch.cat((tfi_layer, img4x), dim=1))

        hr2x = F.interpolate(hr4x, scale_factor=2, mode='bilinear',
                             align_corners=False)  # torch.Size([8, 32, 256, 256])
...
class FusionBranch(nn.Module):
    ...
    def forward(self, img, lr8x, hr2x):
        # print(np.shape(lr8x))128
        lr4x = F.interpolate(lr8x, scale_factor=1, mode='bilinear', align_corners=False)
        # print(np.shape(lr4x))128
        # lr4x = self.conv_lr4x(lr4x)128
        # print(np.shape(lr4x))256
        lr2x = F.interpolate(lr4x, scale_factor=2, mode='bilinear', align_corners=False)  # 256

        # lr2x = F.interpolate(lr2x, scale_factor=2, mode='bilinear', align_corners=False)
        # hr2x = F.interpolate(hr2x, scale_factor=2, mode='bilinear', align_corners=False)
        # print(np.shape(lr2x))
        # print(np.shape(hr2x))
        # print(np.shape(torch.cat((lr2x, hr2x), dim=1)))
...
class MODNet(nn.Module):
    ...
    def __init__(self, in_channels=3, hr_channels=32, backbone_arch='mobilenetv2', backbone_pretrained=True):
        ...
        self.encoder = ViTAE_noRC_MaxPooling_bias_basic_stages4_14(pretrained=False)
        self.enc_channels = [16, 24, 32, 96, 1280]
        self.merged_decoder = mergedDecoder(self.encoder, self.hr_channels, self.enc_channels)
        self.f_branch = FusionBranch(self.hr_channels, self.enc_channels)
